...
const isAuthCallback=()=>{const search=new URLSearchParams(location.search),hash=new URLSearchParams(location.hash.replace(/^#/,'').split('?')[0]);return search.has('code')||search.has('error_description')||hash.has('access_token')||hash.has('error_description')};
async function stageEnquiry(supabase,customerEmail,batch){const {error}=await supabase.from('pending_enquiry_drafts').insert({token:crypto.randomUUID(),customer_email:customerEmail,payload:{drafts:batch}});return error};
...
function run(){contactPanel();guideIndividualButtons();setupVendorLogin()}new MutationObserver(()=>requestAnimationFrame(run)).observe(document.documentElement,{childList:true,subtree:true});window.addEventListener('DOMContentLoaded',()=>{addPrivateTestVendor();run();setTimeout(continueBatch,700);setTimeout(continueBatch,1800);setTimeout(continueVendorLogin,300);const supabase=client();supabase?.auth.onAuthStateChange?.((_event,session)=>{if(session?.user&&isAuthCallback())location.replace('/complete-enquiry')})});
})();
